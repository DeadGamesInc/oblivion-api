...
public class LookupService {
    ...
    public LookupService(ILogger<LookupService> logger, IHttpClientFactory httpFactory, IPinataClient pinataClient) {
        _logger = logger;
    ...
    public async Task<NftMetadataResponse> GetNFTMetadata(string uri) {
        try {
            ...
            if (string.IsNullOrEmpty(uri) || uri.Contains("test.com") || uri.Contains("dawdsawddasws")) return null;
    ...
    public async Task<decimal> GetHistoricalPrice(string id, DateTime? dateTime) {
        try {
            ...
            var price = new HistoricalPrice
                { CoinGeckoKey = id, Price = result.MarketData.CurrentPrice["usd"] ?? 0, PriceDate = date };
    ...
    public async Task PinIPFSCids(List<string> cids) {
        ...
        try {
            ...
            var pinList = await GetPinataPinList();
            var pinnedCids = pinList.Select(a => a.IpfsPinHash).ToList();
            
            cids = cids.Except(pinnedCids).ToList();

            foreach (var cid in cids) {
                var result = await _pinataClient.Pinning.PinByHashAsync(cid);
                if (!result.IsSuccess) {
                    _logger.LogWarning("IPFS CID pinning failed: {Cid} {Code} : {Reason}", result.IpfsHash,
            ...
            _logger.LogInformation("Successfully pinned {Count} IPFS CIDs", successCount);
            _logger.LogError(error, "An exception occurred while pinning IPFS CIDs");
            _exceptions++;
            _totalExceptions++;
        }
    }

    private async Task<List<PinListRow>> GetPinataPinList(int pageLimit = 1000) {
        var resultList = new List<PinListRow>();
        var pageOffset = 0;
        while (true) {
            try {
                var pinList = await _pinataClient.Data.PinList(new { pageLimit, pageOffset });
                if (pinList.Rows.Length == 0) {
                    break;
                }
                resultList.AddRange(pinList.Rows);
                pageOffset += pinList.Rows.Length;
            } catch (Exception ex) {
    ...
    public async Task UpdateNftApi(ChainID chainID, List<string> nfts) {
        try {
            ...
            if (!result.IsSuccessStatusCode) {
                _logger.LogWarning("NFT API update failed: {Code} : {Reason}", result.StatusCode,
                    result.ReasonPhrase);
                _generalErrors++;
                _totalGeneralErrors++;
            }
        } catch (Exception error) {
            _logger.LogError(error, "An exception occurred while updating NFT API");
            _exceptions++;
            _totalExceptions++;
        }
    }
